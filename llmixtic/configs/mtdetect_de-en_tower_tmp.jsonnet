...
local run_name = std.format("mtdetect_%s_%s_tmp", [mt, language]);
...
local dev_data = {
    "mtdetect_dev": {
        dataset_path: std.format("/home/cgarcia/Documentos/mtdetect/mtdetect/wmt_data/%s-mtpaper.all.sentences.shuf.%s.%s.out.all.shuf", [language, "dev", mt]),
        split: "dev",
    },
};
...
local model = {
    ...
    "training_params": {
        "output_dir": std.format("./checkpoints_tmp/%s/training", run_name),
        "save_total_limit": 1,
    ...
    "inference_params": {
        ...
        "output_dir": std.format("./checkpoints_tmp/%s/inference", run_name),
    }
};
